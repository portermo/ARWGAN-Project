...
def main():
    ...
    # 1. 載入設定
    train_options, net_config, _ = utils.load_options(args.options_file)
    
    # 2. 定義攻擊模式
    if args.mode == 'paper':
        print("=== Mode: Paper Reproduction (Jpeg(50) + Crop(3.5%)) ===")
        noise_config = ['Jpeg(50)', 'crop((0.965,0.965),(0.965,0.965))']
    elif args.mode == 'hell':
        print("=== Mode: Hell Mode (5 Noise Attacks) ===")
        noise_config = ['dropout(0.3,0.3)', 'crop((0.965,0.965),(0.965,0.965))', 
                        'resize(0.8,0.8)', 'Jpeg(50)', 'gaussian(3,2.0)']
    ...
    with torch.no_grad():
        for idx, filename in enumerate(image_files):
            ...
            # 從 losses 中提取指標（validate_on_batch 已經計算了 PSNR）
            ...
            if idx == 0:
                # 轉換回 [0, 1] 範圍以便保存
                image_tensor_01 = (image_tensor + 1) / 2
                encoded_images_01 = (encoded_images + 1) / 2
                utils.save_images(image_tensor_01.cpu(), encoded_images_01.cpu(), 
                                'result_example', args.output_folder, resize_to=(128, 128))
                print(f"\nExample images saved to {args.output_folder}/result_example_*.png")
